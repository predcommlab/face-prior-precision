...
for candI = 1:numel(candRDM_stack)
    temp=candRDM_stack{candI};
    %candRDM_stack{candI}= temp(validConds,validConds,:);
    meanCandRDMs(:,:,candI) = nanmean(candRDM_stack{candI},3);
end
...
switch userOptions.RDMrelatednessTest,
    case 'subjectRFXsignedRank'
        ...
        for candI = 1:numel(candRDMs)
            % [ps(candI),h,stats(candI)] = signrank_onesided(cand2refSims(:,candI));
            [ps(candI),h,stats(candI)] = signrank(cand2refSims(:,candI), [],'tail','right','alpha',0.05,'method','approximate');
        end
        ps = ps(sortedIs);
        ...
        es = es(sortedIs);
        stats = stats(sortedIs);
        stats_p_r.stats = stats;
